...
_paw_zoom_flags() {
  COMPREPLY=(--file --rows --cols --offset --col-offset --zoom --charset --quiet --snapshot --live --interval --follow --max-frames)
}
_paw_complete_paw_zoom() {
  local cur prev
  cur="${COMP_WORDS[COMP_CWORD]}"
  if [[ "$cur" == --* ]]; then
    COMPREPLY=($(compgen -W '--file --rows --cols --offset --col-offset --zoom --charset --quiet --snapshot --live --interval --follow --max-frames' -- "$cur"))
    return 0
  fi
  # No subcommands; nothing else to complete.
  return 0
